...
openBrowser a web browser displaying trace:
	gotrace [flags] [pkg.test] trace.out

Generate a pprof-like profile from the trace:
    gotrace -pprof=TYPE [pkg.test] trace.out
...
'gotrace' work on every browser, the trace viewer itself
...
var loader struct {
	once sync.Once
	res  analyzer.ParseResult
	err  error
}

// parseEvents is a compatibility wrapper that returns only
// the Events part of analyzer.ParseResult returned by parseTrace.
func parseEvents() ([]*analyzer.Event, error) {
	res, err := parseTrace()
	if err != nil {
		return nil, err
	}
...
func parseTrace() (analyzer.ParseResult, error) {
	loader.once.Do(func() {
		...
		// Parse and symbolize.
		res, err := analyzer.Parse(bufio.NewReader(tracef), programBinary)
		if err != nil {
...
// browserCommands returns a list of possible commands to use to open a url.
