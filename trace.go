...
import (
	...
	"github.com/bobotu/gotrace/analyzer"
...
func httpTraceViewerHTML(w http.ResponseWriter, r *http.Request) {
	stat, err := pkger.Stat("/static/trace_viewer_full.html")
	if err != nil {
		http.Error(w, err.Error(), 404)
		return
	}
	file, err := pkger.Open("/static/trace_viewer_full.html")
	if err != nil {
		http.Error(w, err.Error(), 404)
		return
	}
	http.ServeContent(w, r, "trace_viewer_full.html", stat.ModTime(), file)
}

func webcomponentsJS(w http.ResponseWriter, r *http.Request) {
	stat, err := pkger.Stat("/static/webcomponents.min.js")
	if err != nil {
		http.Error(w, err.Error(), 404)
		return
	}
	file, err := pkger.Open("/static/webcomponents.min.js")
	if err != nil {
		http.Error(w, err.Error(), 404)
		return
	}
	http.ServeContent(w, r, "webcomponents.min.js", stat.ModTime(), file)
}
...
func httpJsonTrace(w http.ResponseWriter, r *http.Request) {
	...
	if goids := r.FormValue("goid"); goids != "" {
		...
		params.maing = goid
		params.gs = analyzer.RelatedGoroutines(res.Events, goid)
...
func splitTrace(res analyzer.ParseResult) []Range {
	params := &traceParams{
		parsed:  res,
		endTime: math.MaxInt64,
	}
...
type gInfo struct {
	...
	isSystemG  bool
	start      *analyzer.Event // most recent EvGoStart
	markAssist *analyzer.Event // if non-nil, the mark assist currently running.
}
...
func generateTrace(params *traceParams, consumer traceConsumer) error {
	...
	setGState := func(ev *analyzer.Event, g uint64, oldState, newState gState) {
		info := getGInfo(g)
	...
	for _, ev := range ctx.parsed.Events {
		...
		switch ev.Type {
		case analyzer.EvGoStart, analyzer.EvGoStartLabel:
			setGState(ev, ev.G, gRunnable, gRunning)
			info := getGInfo(ev.G)
			info.start = ev
		case analyzer.EvProcStart:
			ctx.threadStats.prunning++
		case analyzer.EvProcStop:
			ctx.threadStats.prunning--
		case analyzer.EvGoCreate:
			newG := ev.Args[0]
			info := getGInfo(newG)
			if info.name != "" {
		...
		case analyzer.EvGoSched, analyzer.EvGoPreempt:
			setGState(ev, ev.G, gRunning, gRunnable)
		case analyzer.EvGoStop,
			analyzer.EvGoSleep, analyzer.EvGoBlock, analyzer.EvGoBlockSend, analyzer.EvGoBlockRecv,
			analyzer.EvGoBlockSelect, analyzer.EvGoBlockSync, analyzer.EvGoBlockCond, analyzer.EvGoBlockNet:
			setGState(ev, ev.G, gRunning, gWaiting)
		case analyzer.EvGoBlockGC:
			setGState(ev, ev.G, gRunning, gWaitingGC)
		case analyzer.EvGCMarkAssistStart:
			getGInfo(ev.G).markAssist = ev
		case analyzer.EvGCMarkAssistDone:
			getGInfo(ev.G).markAssist = nil
		case analyzer.EvGoWaiting:
			setGState(ev, ev.G, gRunnable, gWaiting)
		case analyzer.EvGoInSyscall:
		...
		case analyzer.EvHeapAlloc:
			ctx.heapStats.heapAlloc = ev.Args[0]
		case analyzer.EvNextGC:
		...
		if ctx.gs != nil && ev.P < analyzer.FakeP && !ctx.gs[ev.G] {
			continue
		}
		...
		if ev.P < analyzer.FakeP && ev.P > maxProc {
			maxProc = ev.P
		}
		...
		case analyzer.EvGCStart:
			ctx.emitSlice(ev, "GC")
		case analyzer.EvGCDone:
		case analyzer.EvGCSTWStart:
			if ctx.mode&modeGoroutineOriented != 0 {
				continue
			}
			ctx.emitSlice(ev, fmt.Sprintf("STW (%s)", ev.SArgs[0]))
		case analyzer.EvGCSTWDone:
		case analyzer.EvGCMarkAssistStart:
		...
		case analyzer.EvGoStart, analyzer.EvGoStartLabel:
			info := getGInfo(ev.G)
			if ev.Type == analyzer.EvGoStartLabel {
		...
		case analyzer.EvGoSysCall:
			ctx.emitInstant(ev, "syscall", "")
		case analyzer.EvGoSysExit:
			ctx.emitArrow(ev, "sysexit")
		case analyzer.EvUserLog:
			ctx.emitInstant(ev, formatUserLog(ev), "user event")
		case analyzer.EvUserTaskCreate:
			ctx.emitInstant(ev, "task start", "user event")
		case analyzer.EvUserTaskEnd:
	...
	ctx.emitFooter(&ViewerEvent{Name: "thread_name", Phase: "M", Pid: procsSection, Tid: analyzer.GCP, Arg: &NameArg{"GC"}})
	ctx.emitFooter(&ViewerEvent{Name: "thread_sort_index", Phase: "M", Pid: procsSection, Tid: analyzer.GCP, Arg: &SortIndexArg{-6}})

	ctx.emitFooter(&ViewerEvent{Name: "thread_name", Phase: "M", Pid: procsSection, Tid: analyzer.NetpollP, Arg: &NameArg{"Network"}})
	ctx.emitFooter(&ViewerEvent{Name: "thread_sort_index", Phase: "M", Pid: procsSection, Tid: analyzer.NetpollP, Arg: &SortIndexArg{-5}})

	ctx.emitFooter(&ViewerEvent{Name: "thread_name", Phase: "M", Pid: procsSection, Tid: analyzer.TimerP, Arg: &NameArg{"Timers"}})
	ctx.emitFooter(&ViewerEvent{Name: "thread_sort_index", Phase: "M", Pid: procsSection, Tid: analyzer.TimerP, Arg: &SortIndexArg{-4}})

	ctx.emitFooter(&ViewerEvent{Name: "thread_name", Phase: "M", Pid: procsSection, Tid: analyzer.SyscallP, Arg: &NameArg{"Syscalls"}})
	ctx.emitFooter(&ViewerEvent{Name: "thread_sort_index", Phase: "M", Pid: procsSection, Tid: analyzer.SyscallP, Arg: &SortIndexArg{-3}})
...
func (ctx *traceContext) time(ev *analyzer.Event) float64 {
	// Trace viewer wants timestamps in microseconds.
	return float64(ev.Ts) / 1000
}

func withinTimeRange(ev *analyzer.Event, s, e int64) bool {
...
func (ctx *traceContext) proc(ev *analyzer.Event) uint64 {
	if ctx.mode&modeGoroutineOriented != 0 && ev.P < analyzer.FakeP {
		return ev.G
	} else {
		return uint64(ev.P)
	}
...
func (ctx *traceContext) emitSlice(ev *analyzer.Event, name string) {
	ctx.emit(ctx.makeSlice(ev, name))
}

func (ctx *traceContext) makeSlice(ev *analyzer.Event, name string) *ViewerEvent {
	...
	if ctx.mode&modeTaskOriented != 0 && ev.G != 0 {
		...
		if t := ev.Type; t == analyzer.EvGoStart || t == analyzer.EvGoStartLabel {
			type Arg struct {
				P int
			}
			sl.Arg = &Arg{P: ev.P}
...
func (ctx *traceContext) emitTask(task *taskDesc, sortIndex int) {
	...
	if task.create != nil && task.create.Type == analyzer.EvUserTaskCreate && task.create.Args[1] != 0 {
		ctx.arrowSeq++
...
func (ctx *traceContext) emitHeapCounters(ev *analyzer.Event) {
...
func (ctx *traceContext) emitGoroutineCounters(ev *analyzer.Event) {
...
func (ctx *traceContext) emitThreadCounters(ev *analyzer.Event) {
...
func (ctx *traceContext) emitInstant(ev *analyzer.Event, name, category string) {
	if !tsWithinRange(ev.Ts, ctx.startTime, ctx.endTime) {
		return
	}
	...
	var arg interface{}
	if ev.Type == analyzer.EvProcStart {
		type Arg struct {
			ThreadID uint64
		}
		arg = &Arg{ev.Args[0]}
...
func (ctx *traceContext) emitArrow(ev *analyzer.Event, name string) {
	if ev.Link == nil {
	...
	if ev.P == analyzer.NetpollP || ev.P == analyzer.TimerP || ev.P == analyzer.SyscallP {
		...
		ctx.emitInstant(&analyzer.Event{P: ev.P, Ts: ev.Ts}, "unblock", "")
	}
...
func (ctx *traceContext) stack(stk []*analyzer.Frame) int {
	return ctx.buildBranch(ctx.frameTree, stk)
}

// buildBranch builds one branch in the prefix tree rooted at ctx.frameTree.
func (ctx *traceContext) buildBranch(parent frameNode, stk []*analyzer.Frame) int {
	if len(stk) == 0 {
		return parent.id
	}
	last := len(stk) - 1
