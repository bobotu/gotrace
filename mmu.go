...
var utilFlagNames = map[string]analyzer.UtilFlags{
	"perProc":    analyzer.UtilPerProc,
	"stw":        analyzer.UtilSTW,
	"background": analyzer.UtilBackground,
	"assist":     analyzer.UtilAssist,
	"sweep":      analyzer.UtilSweep,
}

type mmuCacheEntry struct {
	init     sync.Once
	util     [][]analyzer.MutatorUtil
	mmuCurve *analyzer.MMUCurve
	err      error
}

var mmuCache struct {
	m    map[analyzer.UtilFlags]*mmuCacheEntry
	lock sync.Mutex
}

func init() {
	mmuCache.m = make(map[analyzer.UtilFlags]*mmuCacheEntry)
}

func getMMUCurve(r *http.Request) ([][]analyzer.MutatorUtil, *analyzer.MMUCurve, error) {
	var flags analyzer.UtilFlags
	...
	c.init.Do(func() {
		...
		if err != nil {
			...
			c.util = analyzer.MutatorUtilization(events, flags)
			c.mmuCurve = analyzer.NewMMUCurve(c.util)
		}
	})
...
func newLinkedUtilWindow(ui analyzer.UtilWindow, window time.Duration) linkedUtilWindow {
