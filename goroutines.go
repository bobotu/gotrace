...
var (
	gsInit sync.Once
	gs     map[uint64]*analyzer.GDesc
)

// analyzeGoroutines generates statistics about execution of all goroutines and stores them in gs.
func analyzeGoroutines(events []*analyzer.Event) {
	gsInit.Do(func() {
		gs = analyzer.GoroutineStats(events)
	})
}
...
func httpGoroutine(w http.ResponseWriter, r *http.Request) {
	...
	_, ok := reflect.TypeOf(analyzer.GDesc{}).FieldByNameFunc(func(s string) bool {
		return s == sortby
	})
	if !ok {
		sortby = "TotalTime"
...
var templGoroutine = template.Must(template.New("").Funcs(template.FuncMap{
	...
	"unknownTime": func(desc *analyzer.GDesc) int64 {
