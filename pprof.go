...
func pprofByGoroutine(compute func(io.Writer, map[uint64][]interval, []*analyzer.Event) error) func(w io.Writer, r *http.Request) error {
...
func pprofByRegion(compute func(io.Writer, map[uint64][]interval, []*analyzer.Event) error) func(w io.Writer, r *http.Request) error {
	return func(w io.Writer, r *http.Request) error {
...
func pprofMatchingGoroutines(id string, events []*analyzer.Event) (map[uint64][]interval, error) {
	if id == "" {
		return nil, nil
	}
...
func computePprofIO(w io.Writer, gToIntervals map[uint64][]interval, events []*analyzer.Event) error {
	prof := make(map[uint64]Record)
	for _, ev := range events {
		if ev.Type != analyzer.EvGoBlockNet || ev.Link == nil || ev.StkID == 0 || len(ev.Stk) == 0 {
			continue
		}
...
func computePprofBlock(w io.Writer, gToIntervals map[uint64][]interval, events []*analyzer.Event) error {
	prof := make(map[uint64]Record)
	for _, ev := range events {
		switch ev.Type {
		case analyzer.EvGoBlockSend, analyzer.EvGoBlockRecv, analyzer.EvGoBlockSelect,
			analyzer.EvGoBlockSync, analyzer.EvGoBlockCond, analyzer.EvGoBlockGC:
...
func computePprofSyscall(w io.Writer, gToIntervals map[uint64][]interval, events []*analyzer.Event) error {
	prof := make(map[uint64]Record)
	for _, ev := range events {
		if ev.Type != analyzer.EvGoSysCall || ev.Link == nil || ev.StkID == 0 || len(ev.Stk) == 0 {
			continue
		}
		overlapping := pprofOverlappingDuration(gToIntervals, ev)
		if overlapping > 0 {
...
func computePprofSched(w io.Writer, gToIntervals map[uint64][]interval, events []*analyzer.Event) error {
	prof := make(map[uint64]Record)
	for _, ev := range events {
		if (ev.Type != analyzer.EvGoUnblock && ev.Type != analyzer.EvGoCreate) ||
...
func pprofOverlappingDuration(gToIntervals map[uint64][]interval, ev *analyzer.Event) time.Duration {
