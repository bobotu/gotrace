...
func httpUserTask(w http.ResponseWriter, r *http.Request) {
	...
	filter, err := newTaskFilter(r, res)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	type entry struct {
		...
		GCTime, StallTime time.Duration
	}

	base := time.Duration(firstTimestamp()) * time.Nanosecond // trace start
	handleTasks := func(ids []uint64, output chan<- entry, counter *int32) {
		defer func() {
		...
		for _, id := range ids {
			...
			rawEvents := append([]*analyzer.Event{}, task.events...)
			...
			output <- entry{
				...
				StallTime:  task.overlappingGCDuration(res.stallEvents),
			}
		}
	}
	...
	counter := int32(workers)
	chunk := len(tasks)/workers + 1
	for i := 0; i < workers; i++ {
		lo, hi := chunk*i, chunk*(i+1)
		if hi > len(tasks) {
			hi = len(tasks)
		}
		go handleTasks(taskIds[lo:hi], entries, &counter)
	}
...
type annotationAnalysisResult struct {
	...
	gcEvents    []*analyzer.Event             // GCStartevents, sorted
	stallEvents []*analyzer.Event             // Sweep and AssistMark events
}

type regionTypeID struct {
	Frame analyzer.Frame // top frame
	Type  string
}
...
func analyzeAnnotations() (annotationAnalysisResult, error) {
	...
	var gcEvents, stallEvents []*analyzer.Event

	for _, ev := range events {
		switch typ := ev.Type; typ {
		case analyzer.EvUserTaskCreate, analyzer.EvUserTaskEnd, analyzer.EvUserLog:
			taskid := ev.Args[0]
			task := tasks.task(taskid)
			task.addEvent(ev)

			// retrieve parent task information
			if typ == analyzer.EvUserTaskCreate {
		...
		case analyzer.EvGCMarkAssistStart, analyzer.EvGCSweepStart:
	...
	return annotationAnalysisResult{tasks: tasks, regions: regions, gcEvents: gcEvents, stallEvents: stallEvents}, nil
}
...
type taskDesc struct {
	...
	events     []*analyzer.Event   // sorted based on timestamp.
	...
	create *analyzer.Event // Task create event
	end    *analyzer.Event // Task end event
...
func (task *taskDesc) addEvent(ev *analyzer.Event) {
	if task == nil {
		return
	}
...
func (task *taskDesc) overlappingGCDuration(evs []*analyzer.Event) (overlapping time.Duration) {
	start, end := task.firstTimestamp(), task.lastTimestamp()
	idx := sort.Search(len(evs), func(i int) bool { return evs[i].Ts >= start })
	for _, ev := range evs[idx:] {
		if ev.Ts > end {
			break
		}
...
func (task *taskDesc) overlappingInstant(ev *analyzer.Event) bool {
...
// as well.
func (task *taskDesc) overlappingDuration(ev *analyzer.Event) (time.Duration, bool) {
	start := ev.Ts
	end := lastTimestamp()
	if ev.Link != nil {
		end = ev.Link.Ts
...
func (task *taskDesc) lastEvent() *analyzer.Event {
	if task == nil {
		return nil
	}
...
func (task *taskDesc) RelatedGoroutines(events []*analyzer.Event, depth int) map[uint64]bool {
	...
	for i := 0; i < depth; i++ {
		...
		for _, ev := range events {
			...
			if ev.Type == analyzer.EvGoUnblock && gmap[ev.Args[0]] {
				gmap1[ev.G] = true
			}
		}
		gmap = gmap1
...
func newTaskFilter(r *http.Request, res annotationAnalysisResult) (*taskFilter, error) {
	...
	if stall, err := time.ParseDuration(r.FormValue("stall")); err == nil {
		name = append(name, fmt.Sprintf("gc stall >= %s", stall))
		conditions = append(conditions, func(t *taskDesc) bool {
			return t.overlappingGCDuration(res.stallEvents) >= stall
		})
	}
...
func taskMatches(t *taskDesc, text string) bool {
	for _, ev := range t.events {
		switch ev.Type {
		case analyzer.EvUserTaskCreate, analyzer.EvUserRegion, analyzer.EvUserLog:
...
({{if .Complete}}complete{{else}}incomplete{{end}})</td>
	...
	<tr>
		...
		<td></td>
		<td></td>
		<td>GC Window:{{.GCTime}}</td>
	</tr>
	<tr>
		<td></td>
		<td></td>
		<td></td>		
		<td>GC Stall:{{.StallTime}}</td>
	</tr>
    {{end}}
</body>
</html>
...
func formatUserLog(ev *analyzer.Event) string {
...
func isUserAnnotationEvent(ev *analyzer.Event) (taskID uint64, ok bool) {
	switch ev.Type {
	case analyzer.EvUserLog, analyzer.EvUserRegion, analyzer.EvUserTaskCreate, analyzer.EvUserTaskEnd:
...
<th onclick="reloadTable('sortby', 'MarkTime')" class="pause-time"> GC mark</th>
</tr>
{{range .Data}}
  <tr>
	...
	<td> {{prettyDuration .MarkTime}} {{percent .MarkTime .TotalTime}}</td>
  </tr>
{{end}}
</table>
</p>
