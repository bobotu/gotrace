...
type stacks map[uint64][]*analyzer.Frame
...
func (s *stacks) add(fname string) uint64 {
	if *s == nil {
		*s = make(map[uint64][]*analyzer.Frame)
	}

	id := uint64(len(*s))
	(*s)[id] = []*analyzer.Frame{{Fn: fname}}
	return id
}
...
func TestGoroutineCount(t *testing.T) {
	w := analyzer.NewWriter()
	w.Emit(analyzer.EvBatch, 0, 0)  // start of per-P batch event [pid, timestamp]
	w.Emit(analyzer.EvFrequency, 1) // [ticks per second]
	...
	w.Emit(analyzer.EvGoCreate, 1, 10, s.add("pkg.f1"), s.add("main.f1")) // [timestamp, new goroutine id, new stack id, stack id]
	w.Emit(analyzer.EvGoWaiting, 1, 10)                                   // [timestamp, goroutine id]

	// goroutine 20: in syscall
	w.Emit(analyzer.EvGoCreate, 1, 20, s.add("pkg.f2"), s.add("main.f2"))
	w.Emit(analyzer.EvGoInSyscall, 1, 20) // [timestamp, goroutine id]

	// goroutine 30: runnable
	w.Emit(analyzer.EvGoCreate, 1, 30, s.add("pkg.f3"), s.add("main.f3"))

	w.Emit(analyzer.EvProcStart, 2, 0) // [timestamp, thread id]

	// goroutine 40: runnable->running->runnable
	w.Emit(analyzer.EvGoCreate, 1, 40, s.add("pkg.f4"), s.add("main.f4"))
	w.Emit(analyzer.EvGoStartLocal, 1, 40)          // [timestamp, goroutine id]
	w.Emit(analyzer.EvGoSched, 1, s.add("main.f4")) // [timestamp, stack]

	res, err := analyzer.Parse(w, "")
	if err != nil {
...
func TestGoroutineFilter(t *testing.T) {
	...
	w.Emit(analyzer.EvGoCreate, 1, 20, s.add("pkg.f2"), s.add("main.f2"))
	w.Emit(analyzer.EvGoStartLocal, 1, 20)                    // [timestamp, goroutine id]
	w.Emit(analyzer.EvGoUnblockLocal, 1, 10, s.add("pkg.f2")) // [timestamp, goroutine id, stack]
	w.Emit(analyzer.EvGoEnd, 1)                               // [timestamp]

	// goroutine 10: runnable->running->block
	w.Emit(analyzer.EvGoStartLocal, 1, 10)         // [timestamp, goroutine id]
	w.Emit(analyzer.EvGoBlock, 1, s.add("pkg.f3")) // [timestamp, stack]
...
func TestPreemptedMarkAssist(t *testing.T) {
	...
	w.Emit(analyzer.EvGoCreate, 1, 9999, s.add("pkg.f1"), s.add("main.f1")) // [timestamp, new goroutine id, new stack id, stack id]
	w.Emit(analyzer.EvGoStartLocal, 1, 9999)                                // [timestamp, goroutine id]
	w.Emit(analyzer.EvGCMarkAssistStart, 1, s.add("main.f1"))               // [timestamp, stack]
	w.Emit(analyzer.EvGoPreempt, 1, s.add("main.f1"))                       // [timestamp, stack]
	w.Emit(analyzer.EvGoStartLocal, 1, 9999)                                // [timestamp, goroutine id]
	w.Emit(analyzer.EvGCMarkAssistDone, 1)                                  // [timestamp]
	w.Emit(analyzer.EvGoBlock, 1, s.add("main.f2"))                         // [timestamp, stack]
