...
func (g *GDesc) snapshotStat(lastTs, activeGCStartTime int64) (ret GExecutionStat) {
	...
	if g.blockMarkTime != 0 {
		ret.MarkTime += lastTs - g.blockMarkTime
	}
	return ret
}
...
func GoroutineStats(events []*Event) map[uint64]*GDesc {
	...
	for _, ev := range events {
		...
		case EvGCMarkAssistStart:
			g := gs[ev.G]
			if g != nil {
				g.blockMarkTime = ev.Ts
			}
		case EvGCMarkAssistDone:
			g := gs[ev.G]
			if g != nil && g.blockMarkTime != 0 {
				g.MarkTime += ev.Ts - g.blockMarkTime
				g.blockMarkTime = 0
			}
		case EvGCStart:
			gcStartTime = ev.Ts
